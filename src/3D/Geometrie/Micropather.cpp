...
void PathNodePool::Clear()
{
    ...
    // Set up for new allocations (but don't do work we
    // don't need to. Reset/Clear can be called frequently.)
    if ( nAllocated > 0 )
    {
...
PathNode* PathNodePool::GetPathNode( unsigned frame, void* _state, float _costFromStart, 
    ...
    else if ( node->cacheIndex < 0 )
    {
        ...
        if ( node->numAdjacent > 0 )
        {
            ...
            for ( unsigned i=0; i<stateCostVecSize; ++i )
            {
                ...
                pNodeCostPtr[i].node = pathNodePool.GetPathNode(
                            frame, state, FLT_MAX, FLT_MAX, 0 );
            }

            // Can this be cached?
            int start = 0;
            if ( pNodeCost->size() > 0 && pathNodePool.PushCache(
                     pNodeCostPtr, pNodeCost->size(), &start ) )
            {
